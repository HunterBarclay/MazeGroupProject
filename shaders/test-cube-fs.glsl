...
void main(void) {
    ...
    float ambientOcc = length(texture2D(uAmbientMap, vec2(vTextureCoord.s, vTextureCoord.t)).rgb) / sqrt(3.0);

    vec3 textNorm = texture2D(uNormal, vec2(vTextureCoord.s, vTextureCoord.t)).rgb;
